...
## 9. Implementation Checklist (Critical & High Priority)

Before deployment, the coding agent must address the following items in order of priority:

### Critical Issues (Will Break Functionality)

- [ ] **Form Data Payload Handling:** Implement dual-mode payload parsing in `POST /api/logs` endpoint
  - Accept both `application/json` (from CLI tool)
  - Accept `application/x-www-form-urlencoded` (from HTMX dashboard form)
  - Parse and validate both formats against `TimeLogCreate` schema

- [ ] **Client Selection in Dashboard:** Add `<select name="client_id">` dropdown to the logging form
  - Populate with active clients from `/api/clients` endpoint
  - Make `client_id` a required form field
  - Test that form submission includes the selected client ID

- [ ] **Invoice Number Race Condition:** Implement atomic invoice numbering
  - Replace simple count with year-scoped filtering to get accurate invoice count
  - Add exception handling for unique constraint violations
  - Test concurrent invoice generation to verify no duplicates occur

### Important (Production/Deployment Quality)

- [ ] **Docker Layer Caching:** Create `.dockerignore` file to exclude `data/` and `backups/` directories
  - This prevents database file changes from invalidating Docker build cache

- [ ] **SQLite WAL Mode:** Verify `database.py` initializes WAL mode via SQLAlchemy event listener
  - Required for safe concurrent access and backup operations

- [ ] **Form Reset After Submit:** Confirm form clears after successful HTMX swap
  - Verify `hx-on::after-request` handler fires correctly

- [ ] **Git SSH Configuration for Cron:** Document and test root SSH access to Git repository
  - Add root's public key to Git host before deploying backup script
  - Verify `git push` works when executed via `sudo crontab`

### Nice-to-Have (Code Quality)

- [ ] **Import Validation:** Ensure `from datetime import date, timedelta` is present in `services.py`
- [ ] **API Route Deduplication:** Verify final API route table has no duplicate endpoints
- [ ] **Client Dropdown Initialization:** Optionally add HTMX trigger to load clients on page load
